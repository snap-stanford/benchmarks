...
//  cerr<<"Table loaded "<<currentDateTime()<<endl;
  ...
  TVec<TPair<TStr, TAttrType>, int64 > S = P->GetSchema();
  ...
  //Use the following lines if you want to use sequential ToNetwork function
  //  TInt64V crossnetids;
  ...
  //
  //  TIntStrStrTr64V nodeattrmapping;
  //  TIntStrStrTr64V edgeattrmapping;
  ...
  cerr<<"TriangleCnt starting"<<currentDateTime()<<"\n";
  start = clock();
  cerr<<TSnap::GetTriangleCnt(pneanet);
  end = clock();
  tim = (double) (end - start) / CLOCKS_PER_SEC;
  printf("TriangleCnt, %f\n", tim);
  cerr<<"TriangleCnt done"<<currentDateTime()<<"\n";
